...
let state={jobs:[],running:false,settings:{}},selected=null,queueKey='',detailKey='',uploading=false,timer;
...
async function refresh(){try{state=await api('/api/state');render()}catch{$('queueNote').textContent='Server disconnected.'}}
function render(){
 if(!state.jobs.some(j=>j.id===selected))selected=state.jobs[0]?.id??null;
 ...
 const key=JSON.stringify([selected,state.jobs.map(j=>[j.id,j.status,j.stage])]);
 if(key!==queueKey){queueKey=key;$('queue').innerHTML=state.jobs.length?state.jobs.map(j=>`<div class="row ${selected===j.id?'selected':''}" role="button" tabindex="0" data-select="${j.id}" aria-label="View ${escapeHtml(j.name)}"><img src="/api/videos/${j.id}/poster.jpg" alt=""><div class="row-info"><span class="row-name">${escapeHtml(j.name)}</span><div class="row-meta">${duration(j.duration)} · ${(j.bytes/1048576).toFixed(1)} MB</div></div><span class="badge ${j.status}">${j.status==='completed'?'Ready':j.status==='processing'?'Processing':j.status==='queued'?'Queued':j.status==='failed'?'Failed':'Cancelled'}</span><button class="remove" data-id="${j.id}" data-action="${['queued','processing'].includes(j.status)?'cancel':'delete'}" aria-label="${['queued','processing'].includes(j.status)?'Cancel':'Remove'} ${escapeHtml(j.name)}">×</button></div>`).join(''):'<div class="empty">No videos added.</div>'}
...
function result(j){
 if(j.status==='completed')return `<h3>Description</h3><p>${escapeHtml(j.description)}</p><div class="tags"><span>Suspicious: ${escapeHtml(j.suspicious??'unknown')}</span><span>Weapon: ${escapeHtml(j.weapon??'unknown')}</span><span>${j.timestamps.length} frames · ${j.elapsed}s${j.personLabels?' · Person labels':''}</span></div><div class="result-actions"><button data-copy="${j.id}">Copy</button><button data-download="${j.id}">Download</button></div>`;
...
function settingsNote(){const local=$('mode').value==='local';$('personLabelsLabel').hidden=!local;$('modelLabel').style.display=local?'block':'none';$('model').required=local;$('settingsNote').textContent=local?`Custom prompt · ${state.frameLimit} frames · 600 output tokens. Smaller models may produce different results.`:'Uploads videos to your running notebook’s /api/describe endpoint.'}
...
$('settingsForm').onsubmit=async e=>{e.preventDefault();try{await api('/api/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode:$('mode').value,endpoint:$('endpoint').value,model:$('model').value,personLabels:$('personLabels').checked})});$('settings').close();await refresh();toast('Settings saved.')}catch(e){$('settingsError').textContent=e.message}};
